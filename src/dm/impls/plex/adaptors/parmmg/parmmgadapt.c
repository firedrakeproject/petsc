  ...
  if (numProcs > 1) {
    ...
    for (i=vStart-pStart, k=0; i<vEnd-pStart; ++i) {
      if (usedCopies[i]>1 && degree[i]) {
        PetscCall(PetscSectionGetOffset(rankSection, i+pStart, &offset));
        if (vertexNumber[i-vStart+pStart]) rankArray[offset++] = rank;
        for (j=0; j<degree[i]; j++, mrl++) {
          if (rankOfUsedMultiRootLeafs[mrl]) {
            rankArray[offset++] = rankOfUsedMultiRootLeafs[mrl] - 1;
          }
        }
      } else mrl += degree[i];
    }
