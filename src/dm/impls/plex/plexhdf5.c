  ...
  *distdm = NULL;
  ...
  /* Migrate DM */
  {
    ...
    if (PetscDefined(USE_DEBUG)) {
      for (p = 0; p < *chartSize; ++p) {PetscCheck(buffer2[p].rank >= 0, PETSC_COMM_SELF, PETSC_ERR_ARG_WRONG, "Found negative root rank %" PetscInt_FMT " at local point %" PetscInt_FMT " on rank %" PetscInt_FMT " when making migrationSF", buffer2[p].rank, p, rank);}
    }
  ...
  /* Set pointSF */
  {
    ...
    if (PetscDefined(USE_DEBUG)) {
      for (p = 0; p < *chartSize; ++p) {PetscCheck(buffer0[p].rank >= 0, PETSC_COMM_SELF, PETSC_ERR_ARG_WRONG, "Found negative root rank %" PetscInt_FMT " at local point %" PetscInt_FMT " on rank %" PetscInt_FMT " when making pointSF", buffer0[p].rank, p, rank);}
    }
  ...
  PetscCall(DMPlexReorderSetDefault(*distdm, PETSC_FALSE));
  PetscCall(PetscViewerHDF5PopGroup(viewer));
  PetscFunctionReturn(0);
}
