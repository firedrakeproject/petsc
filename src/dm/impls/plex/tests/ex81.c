...
PetscErrorCode ProcessOptions(MPI_Comm comm, AppCtx *options)
{
  ...
  PetscCall(PetscOptionsBoundedInt("-test_num", "The test #", "ex81.c", options->testNum, &options->testNum, NULL, 0));
  PetscCall(PetscOptionsBoundedInt("-subdim", "The mesh to create", "ex81.c", options->subdim, &options->subdim, NULL, 0));
  PetscCall(PetscOptionsBoundedInt("-overlap", "The partition overlap", "ex81.c", options->overlap, &options->overlap, NULL, 0));
  PetscOptionsEnd();
...
/*TEST
  ...
  testset:
    ...
    test:
      suffix: 0
      args: -test_num 0 -subdim 2 -overlap 0

    test:
      suffix: 1
      args: -test_num 0 -subdim 1 -overlap 0

    test:
      suffix: 2
      args: -test_num 0 -subdim 0 -overlap 0

    test:
      suffix: 3
      args: -test_num 0 -subdim 2 -overlap 1

    test:
      suffix: 4
      args: -test_num 0 -subdim 1 -overlap 1

    test:
      suffix: 5
      args: -test_num 0 -subdim 0 -overlap 1
